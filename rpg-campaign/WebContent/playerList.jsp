...
<%@ page import="java.util.List, br.com.rpg.campaign.user.model.Player"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<c:url value="/remove-user" var="linkRemoveUser" />
<c:url value="/user-details" var="linkUserDetails" />
<c:url value="/show-user" var="linkShowUser" />
...
<body class="w3-black">
	...
	<div class="w3-padding-large" id="main">
			...
			<ul>
				<c:forEach items="${playerList}" var="player">
					<li>${player.id} - ${player.username} - ${player.email} - <a
						href="${linkUserDetails}?id=${player.id}">[ detalhes ]</a> -
						<a href="${linkShowUser}?id=${player.id}">[ editar ]</a> -
						<a href="${linkRemoveUser}?id=${player.id}">[ remover ]</a></li>
				</c:forEach>
			</ul>
		...
		<%
			System.out.println("[Log] : Página playerList acessada!");
		%>
		<!-- END PAGE CONTENT -->
	</div>
