...
<%@ page
	import="java.util.List, br.com.rpg.campaign.campaign.model.Campaign"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<c:url value="/remove-campaign" var="linkRemoveCampaign" />
<c:url value="/show-campaign" var="linkEditCampaign" />
<c:url value="/campaign-details" var="linkCampaignDetails" />
...
<body class="w3-black">
	...
	<div class="w3-padding-large" id="main">
			...
			<ul>
				<c:forEach items="${campaignList}" var="campaign">
					<li>${campaign.id} - ${campaign.campaignName} -
						${campaign.dungeonMaster.username} - <a
						href="${linkCampaignDetails}?id=${campaign.id}">[ detalhes ]</a> - <a
						href="${linkEditCampaign}?id=${campaign.id}">[ editar ]</a> - <a
						href="${linkRemoveCampaign}?id=${campaign.id}">[ remover ]</a>
					</li>
				</c:forEach>
			</ul>
		...
		<%
			System.out.println("[Log] : Página campaignList acessada!");
		%>
		<!-- END PAGE CONTENT -->
	</div>
